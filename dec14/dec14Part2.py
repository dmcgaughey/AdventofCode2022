...
rock_map = np.zeros([max_y+1+1, max_x-min_x+1],dtype=int)
...
# Now simulate the sand falling (Part 2: When reaches bottom will place a rock)
done = False
num_rocks = 0
while (not done):
	cur_pos = (500-min_x, 0)
	while (1):
		...
		if rock_map[cur_pos[1]+1,cur_pos[0]] <= 0:
			cur_pos = (cur_pos[0], cur_pos[1]+1)
			continue

		# Check down left
		if rock_map[cur_pos[1]+1,cur_pos[0]-1] <= 0:
		...
		if cur_pos[0]==500-min_x and cur_pos[1]==0: 	done =  True
		break


print(f'Part 2: {num_rocks}')
print_map(rock_map)